...
export default class DeckConfig {
    ...
    getCardName(value) {
        for (const x of [...this.#specialCards, ...this.#suitPictureCards]) {
    ...
    set suits(suits) {
        this.#suits = new SuitCollection(suits);
    }
    ...
    get suitPictureCards() {
        return this.#suitPictureCards;
    }

    set suitPictureCards(cards) {
        this.#suitPictureCards = [];
        for (const str of cards) {
            const [symbol, name] = str.split(':');
            this.#suitPictureCards.push({symbol, name});
        }
    }
    ...
    set specialCards(cards) {
        ...
        for (const str of cards) {
            const [symbol, name] = str.split(':');
            this.#specialCards.push({symbol, name});
        }
    }
    ...
    set specialBids(bids) {
        ...
        for (const str of bids) {
            ...
            this.#specialBids.push({symbol, points, name});
        }
    }
