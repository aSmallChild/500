...
export default class Game {
    ...
    onUserSession(user, session) { // todo call this
        this.users.add(session);

        const player = this.getOrMaybeEvenCreatePlayerForUser(user);
        if (!player) {
            this.onObserverConnect(session);
            // todo sort this out one day, users that join after a game starts, become ghosts
            return;
        }
        this.onPlayerConnect(player, session);
        session.onMessage((event, data) => this.onPlayerMessage(user, session, event, data));
    ...
    onObserverMessage(session, event, data) {
        if  (event == 'stage:mounted') {
            ...
            this.currentStage.onObserver(session);
        }
    }
    ...
    createPlayerForUser(user) {
        const player = new Player(user.name, user);
