...
export default class Round extends GameStage {
    ...
    endRound() {
        // todo write this in a game rule (misere and blind misere not implemented)
        const scores = this.players.map(() => 0);
        this.pastTricks.forEach(trick => scores[trick.winner.position]++);
        scores.forEach((trickCount, i) => {
            if (i == this.winningBidder.position) {
                scores[i] = trickCount >= this.winningBid ? this.winningBid.points : -this.winningBid.points;
            }
            else {
                scores[i] *= 10;
            }
        });
        this.emitStageMessage('scores', scores);
        this.complete({
