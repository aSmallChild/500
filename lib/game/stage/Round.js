...
export default class Round extends GameStage {
    ...
    #playCard(player, socket, actionData) {
        ...
        if (this.currentTrick.length == this.players.length) {
            ...
            this.emitStageMessage('round_scores', roundScores);
            const playerScores = this.calculatePlayerScores(roundScores);
            this.emitStageMessage('player_scores', playerScores);
    ...
    endRound() {
        const roundScores = this.calculateRoundScores();
        const playerScores = this.calculatePlayerScores(roundScores);
        const winner = playerScores[this.winningBidder.position] >= this.config.targetScore ? this.winningBidder : null;
        this.complete({
