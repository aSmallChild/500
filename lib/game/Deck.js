...
export const cardsFromString = (str, config) => {
    const cards = [];
...
export const compareCards = (a, b, config) => {
    const specials = config.specialCards;
    const aSpecialIndex = config.getSymbolIndex(specials, a.value);
    const bSpecialIndex = config.getSymbolIndex(specials, b.value);
    ...
    const pictures = config.suitPictureCards;
    const aPictureIndex = config.getSymbolIndex(pictures, a.value);
    const bPictureIndex = config.getSymbolIndex(pictures, b.value);
...
export const getStartEndNumberCards = (numberOfNumberCards = 9) => {
    let lowest = 2;
    let highest = 10;
...
export const buildSuitPictureCards = (suit, config, pictureCards, cards = [], limit = pictureCards.length) => {
    let count = 0;
    if (count === limit) {
        return cards;
    }
...
export const buildSuitNumberCards = (suit, config, lowest, highest, cards = []) => {
